...
sidebar_position: 3
description: Backend API endpoints the FarmUI-Admin frontend consumes — methods, paths, request bodies, response types, and data models.
---

# FarmUI Admin — Tổng hợp API mà Frontend sử dụng

Tài liệu này liệt kê chi tiết toàn bộ các API endpoint backend mà Frontend (Admin UI) sử dụng, bao gồm method trong code, endpoint thực tế, request body và response structure. Tất cả các API này được định nghĩa tại `src/api/services.ts`.
...
Được định nghĩa chi tiết tại `src/types.ts`.
